class LinkedList { // 노드 클래스를 감싸는 Linked List 라는 클래스를 생성
    ...
    void removeDups() { // 클래스 안에 내부 함수로 선언 -> 나중에 계속 사용하기 위함
        Node n = header; // 가장 먼저 n을 선언
        while (n != null && n.next != null) {// n은 첫번째 노드에서 부터 n.next가 null이 될 때까지 마지막 노드까지 돎
            Node r = n; // 안에서 러너 하나 선언/ 러너는 항상 n에 위치에서 시작
            while (r.next != null) { // 러너도 마찬가지로 마지막 노드까지 돎
                if (n.data == r.next.data) { // 돌면서 데이터가 중복이 되는지, n의 데이터랑 자기가 돌면서 만나는 데이터가 같은지 확인
                                             // next.data를 비교하는 이유는 while문을 보면 러너는 next가 null이 아닌 노드까지만 가기 때문임
                                             // 그 말은 마지막 노드에는 가지 않는다는 뜻임
                                             // 러너가 n과 같은 위치에서 시작함/ n이 러너한테 한바퀴 돌고와라 시키면 러너는 바로 달려가는 것이 아니라
                                             // n이랑 같은 위치에서 다음 노드의 값이 뭔지 보고 n이랑 같으면 내 앞에 노드를 삭제해야 됨
                                             // 그때 내가 삭제해야 될 노드 안에 있는 것보다 그 전에 있는게 좋음
                                             // -> 전에 애가 가지고 있는 다음 주소를 그냥 다다음으로 갈아치우면 되고 그러면 리스트에서 빠짐
                    r.next = r.next.next; // 같으면 자기 다음 주소를 다다음 주소로 갈아 치움
                } else { // 만약에 중복이 안된다면
                    r = r.next; // 빠르게 다른 노드로 그냥 이동
                }
            } // 이렇게 러너가 한바퀴 돌고 오면
            n = n.next; // n은 다음 주소로 가서 러너를 또 돌림
        }
    }
...
public class RemoveDups {
    public static void main(String[] args) {
        ...
        ll.append(2); // 데이터를 append함
        ll.retrieve(); // 출력
        ll.removeDups(); // 만든 함수 실행
        ll.retrieve(); // 출력
    } // 한가지 구멍이 존재/ 마지막 노드가 중복돼서 지워야 할 때 에러 발생
      // 왜냐하면 러너가 마지막 노드가 중복돼서 지워 버렸는데 n이 마지막 노드라고 치면
      // 마지막 노드가 중복돼서 지워버렸는데 n은 마지막 노드/ 그런데 그 마지막 노드는 null임
      // while문이 null.next가 null인지를 비교하려고 노드 객체가 아닌데 접근하려고 하니까 에러 발생
      // 그래서 while문 조건에 n!=null을 추가 해줘야 함
}